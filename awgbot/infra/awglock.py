...
НЕТ ФАЙЛА СОСТОЯНИЯ — хост на поколении GENERATION_FLOOR (1). Файл пишет
установщик: на первой установке — поколение поставки (сервер создан под её
ядро), на обновлении без файла — floor: такое обновление приехало с версии
старше манифеста, а все домантифестные установки — первое поколение по
определению. Считать в этом случае поколение поставки нельзя: обновления
прыгают через ступени, и хост с v2.9 на gen-2 поставке усыновил бы 2 без
переезда. Отсутствие файла у работающего бота — сбой установщика; лучше
лишний раз объявить переезд, чем молча его пропустить.
"""
...
# Поколение всех установок до появления манифеста (v2.10.0): с него начался счёт.
GENERATION_FLOOR = 1
