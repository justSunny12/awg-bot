...
def rebuild_chain(client_ids, *, mark_in_set: bool = False) -> None:
    ...
    mark_in_set переворачивает смысл набора, и это единственное различие двух
    режимов на уровне правил:

      False (умолчание «домой») — метим то, чего в наборе НЕТ. Российский адрес
        достаётся всему, что не заблокировано, включая сервисы на .com, и вести
        перечень российских сайтов не приходится вовсе.
      True (умолчание «за границу») — метим то, что в наборе ЕСТЬ. На шлюз идут
        только перечисленные российские сервисы, всё прочее выходит зарубежным
        адресом.

    Пустой набор в этих режимах означает противоположное: при False на шлюз
    уедет ВЕСЬ трафик включённых, при True — ничего, то есть то же самое, что
    выключенная функция. Отсюда и сторож наполненности списков нужен только в
    первом.
    """
    chain = config.ROUTING_CHAIN
    _mangle(["-N", chain], check=False)          # уже есть → код 1, это норма
    _mangle(["-F", chain])
    negate = [] if mark_in_set else ["!"]
    for cid in sorted(client_ids):
        _mangle(["-A", chain,
                 "-m", "set", "--match-set", src_set(cid), "src",
                 "-m", "set", *negate, "--match-set", user_set(cid), "dst",
