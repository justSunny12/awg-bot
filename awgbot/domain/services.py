...
class Services:
    ...
    # Плохих замеров подряд до ГАШЕНИЯ маркировки. Зависит от режима, потому что
    # цена ожидания в них разная.
    #
    ...
    # «не тот адрес», а отсутствие связи, и каждый лишний такт — это лишние
    # полминуты без интернета у всех включённых. Поэтому гасим по первому же
    # плохому замеру.
    #
    # При умолчании «за границу» помечены только российские сервисы, и то же
    # ожидание стоит «Озон недоступен минуту». Тут дребезг дороже задержки, и
    # три такта уместны.
    #
    # Одиночный плохой замер — уже не шум: внутри него зонд делает две попытки к
    # двум целям с таймаутом 4 с, то есть подтверждает отказ секундами, а не
    # одним потерянным пакетом.
    _RT_DOWN_STREAK_HOME = 1
    _RT_DOWN_STREAK_ABROAD = 3
    ...
    def _rt_down_streak(self) -> int:
        """Плохих замеров подряд до гашения маркировки — по текущему режиму."""
        if config.routing_default_route() == config.ROUTING_DEFAULT_ABROAD:
            return self._RT_DOWN_STREAK_ABROAD
        return self._RT_DOWN_STREAK_HOME
    ...
    def routing_liveness_tick(self) -> list[Notification]:
        ...
        # несколькими замерами. В режиме «домой» гашение при этом происходит
        # раньше объявления — и это правильно: чинить связь важнее, чем
        # рассказывать о ней.
