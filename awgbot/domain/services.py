...
class Services:
    ...
    def routing_liveness_tick(self) -> list[Notification]:
        ...
        if not engaged:
            ...
            # выключение фичи админом (или неготовность списков) — не дребезг, и
            # ждать три такта тут значит не выполнить прямое указание. Раньше
            # разницы не было: «выключено» выражалось тем же плохим вердиктом, а
            # порог гашения равнялся единице, и оба пути совпадали.
