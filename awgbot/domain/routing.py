...
# Похоже на IPv4 — отсекаем: принимаем ТОЛЬКО доменные имена. CIDR и адреса dnsmasq через `ipset=` не обрабатывает в принципе.
...
def build_dnsmasq_conf(
    *,
    ...
    Здесь домен получает одну директиву со списком наборов ВСЕХ, кому он нужен:
        ipset=/gosuslugi.ru/vpn_u2,vpn_u7     (базовый — всем включённым)
        ipset=/lk.bank.ru/vpn_u2              (личный — только этому)

    Правило маркировки тогда одно: «в моём наборе → на шлюз». Проверять
    пересечение двух наборов не нужно.

    Директива действует на домен И ВСЕ ЕГО ПОДДОМЕНЫ — так работает dnsmasq
    (`--ipset`: «Domains and subdomains are matched in the same way as
    --address»), и сопоставление идёт по целым меткам. Поэтому `ozon.ru`
    накрывает `seller.ozon.ru`, но не `myozon.ru`, а `lk.bank.ru` не накрывает
    `bank.ru`. Это и есть контракт для пользователя: дал корень — забрал всё
    под ним, дал поддомен — забрал только его ветку.
    ...
    lines = [
        ...
        "# Домены, которым нужен РОССИЙСКИЙ адрес: они и уходят на домашний шлюз.",
        "# Всё остальное выходит зарубежным адресом обычным путём.",
        "# У каждого профиля свой набор: базовый список + личные добавления.",
        "# Директива действует на домен и все его поддомены.",
        "",
    ]
