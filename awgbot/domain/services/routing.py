"""
routing.py — условная маршрутизация (концепт «условная маршрутизация»):
...
class RoutingMixin:
    ...
    def routing_allowed_for(self, client) -> bool:
        ...
        if client.routing_allowed:
            return True
        # держатель чужих устройств: разрешение — у их владельца (концепт «гость»)
    ...
    def routing_liveness_tick(self) -> list[Notification]:
        ...
        (десятки секунд). концепт «резервный шлюз» §5.
    ...
    def routing_cold_start(self) -> Optional[object]:
        """Выбор активного на старте (концепт «резервный шлюз» §5). Тёплый
