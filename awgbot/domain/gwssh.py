...
Три вещи, которых нет у основного бота:
  ...
  • фильтр снаружи (цепочка ssh_in) — только для порта SSH и только для
    ...
    (peer_nets4, из бандла) открыты всегда, поэтому запереться нельзя и
    таймер отката не нужен. Имена DynDNS резолвит агент
...
class GwSshMixin:
    ...
    def ssh_screen(self, info: dict | None = None, fact: tuple | None = None,
        ...
        return {
            ...
            "peer_nets": sorted((info or {}).get("sets", {}).get("peer_nets4", set())),
