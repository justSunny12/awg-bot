"""
gwownlists.py — свои списки, общие для всех шлюзов (концепт «синхронизация
своих списков», этап 2).

Свои списки локальной сети без VPN («в туннель» / «напрямую») ведутся на
каждом шлюзе в чате его агента; с этого этапа они одни на все шлюзы админа:
...
def merge(canon: dict, slot_id: int, run: str, events, upto, now_iso: str, deny=()) -> tuple:
    """Слияние событий слота в канон — таблица §2.5 концепта.
