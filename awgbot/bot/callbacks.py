...
class BroadcastCB(CallbackData, prefix="bc"):
    ...
    ref — id профиля-адресата; 0 означает «всем». Одна пара хендлеров на оба
    случая: адресная рассылка отличается от общей ровно списком получателей и
    экраном возврата, а весь путь — ввод, превью, проверка разметки, кулдаун,
    отчёт — совпадает буквально. Разводить его на две копии значило бы чинить
