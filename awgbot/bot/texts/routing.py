"""Условная маршрутизация и шлюз — сторона основного бота (концепт «условная маршрутизация»)."""
...
def channel_drift_block(ch: dict | None) -> str:
    ...
    if ch.get("online") and not ch.get("drift_bundle"):
        ...
        todo = ("\nПеревыпусти файл и примени его на шлюзе — или дождись канала: когда он "
                "поднимется, изменения доедут сами.")
...
def gateway_lan_ask(state: dict, on: bool, resolver: str) -> str:
    """Диалог «за шлюзом — без VPN» (концепт «локальная сеть» §6): что произойдёт,
...
def gateway_peer_ask(on: bool) -> str:
    """Диалог тумблера функции B (концепт «локальная сеть» §6)."""
    if on:
