...
def routing_panel_text(*, master_on: bool, domains: list, devices: int,
    ...
    фактом, а не списком с галочками: выбирать там нечего. Состояние шлюза здесь
    НЕ показываем — оно уехало в общий статусный блок главного меню, чтобы
    человек видел его сразу, а не заходя в раздел.
    """
    head = f"<b>🇷🇺 {ROUTING_NAME}</b>\n"
    word = plural_ru(devices, 'устройство', 'устройства', 'устройств')

    if not master_on:
        state = (f"\n❌ Выключено на всех твоих устройствах "
                 f"({devices} {word}).\n" if devices else "\n❌ Выключено.\n")
        return head + state + "\n" + ROUTING_ABOUT_OFF

    if devices:
        state = f"\n✅ Включено на всех твоих устройствах ({devices} {word}).\n"
    else:
        state = "\n✅ Включено. Устройств пока нет — добавь, и режим заработает сразу.\n"
...
def broadcast_prompt(target_name: str = "") -> str:
    """Приглашение ввести текст. Пустое имя — объявление всем.

    Адресата называем прямо в заголовке, а не намёком: экран ввода выглядит
    одинаково независимо от того, откуда пришли, и перепутать «всем» с «одному
    профилю» после переключения между вкладками очень легко. Цена ошибки
    несимметрична — лишний адресат объявление уже прочитал.
    """
    if not target_name:
        return ("📢 <b>Объявление всем</b>\n\nПришли текст — он уйдёт всем "
                "клиентам и друзьям (кроме тебя). Можно с разметкой "
                "(<b>жирный</b>, ссылки). Следующим сообщением покажу превью.")
    return (f"📢 <b>Объявление профилю «{_e(target_name)}»</b>\n\nПришли текст — "
            "он уйдёт владельцу профиля и тем, кому он раздал устройства. "
            "Остальные его не увидят. Можно с разметкой (<b>жирный</b>, ссылки). "
            "Следующим сообщением покажу превью.")


def broadcast_preview(text: str, n: int, target_name: str = "") -> str:
    who = "получателю" if n == 1 else "получателям"
    scope = (f" профиля «{_e(target_name)}»" if target_name else "")
    return (f"📢 <b>Превью объявления</b> (так его увидят):\n\n{text}\n\n"
            f"— — —\nБудет отправлено <b>{n}</b> {who}{scope}. Отправляем?")
