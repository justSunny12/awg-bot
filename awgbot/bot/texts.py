...
def migration_promoted(iface: str) -> str:
    ...
    return (f"🚚 Переезд завершён: основным интерфейсом стал <code>{_e(iface)}</code>, "
            ...
            "деплой-значения читаются при старте.")
...
SETTINGS_SVC = ("🔄 <b>Обслуживание</b>\n\n"
                "Здесь то, что открывают не ради настройки, а когда чинят или "
                "проверяют:\n"
                ...
                "Отсюда же идёт переезд профилей, когда он настроен.")
...
def settings_server_text(d: dict) -> str:
    ...
    lines = [
        ...
        "",
        "Порт и подсеть — у живого интерфейса: именно они уезжают в ссылки. "
        "Порт линка до шлюза здесь ни при чём, он свой и виден в разделе "
        "маршрутизации.",
        "",
