...
def broadcast_preview_photos(n: int, names: list = (),
    ...
    стоит, отправить можно как есть, а текст (или недогруженные снимки)
    вливаются в него сами, как только доедут.
    """
    who = "человеку" if n == 1 else "людям"
    scope = f"\n{_bc_audience(list(names), with_friends)}." if names else ""
    ...
    if not has_text:
        head += ("\n\n✍️ Текста в нём нет — уйдут только картинки. Хочешь с "
                 "текстом — пришли его сообщением, добавлю подписью. Если "
                 "что-то ещё грузится, превью пересоберётся само — подожди.")
    return (f"{head}\n\n"
...
def broadcast_report(names: list, with_friends: bool, delivered: int,
    ...
    if not text:
        return f"{line}\n\nТекста в объявлении не было — только картинки."
