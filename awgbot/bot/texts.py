...
def subscription_block(client, *, for_admin: bool = False, show_pause: bool = True) -> str:
    ...
    from awgbot.core import blocks; from awgbot.core import config as _cfg
    ...
    if show_pause and client.period_kind == PeriodKind.YEAR:
        ...
        avail = max(0, int(_cfg.PAUSE_MAX_TOTAL_DAYS) - used)
        until = f" до {timeutil.fmt_dt(end)}" if end else ""
        lines.append(f"Приостановка: доступно {avail}/{_cfg.PAUSE_MAX_TOTAL_DAYS} дн.{until}")
    return "\n".join(lines)
...
def pause_unavailable() -> str:
    ...
    from awgbot.core import config as _cfg
    return ("Приостановка сейчас недоступна: она возможна только для годовой "
            f"подписки, и суммарно не более {_cfg.PAUSE_MAX_TOTAL_DAYS} дней за период.")
