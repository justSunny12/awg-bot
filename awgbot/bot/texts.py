...
def gateway_removed(dev) -> str:
    return (f"🛑 «{_e(dev.name)}» больше не шлюз, ключи линка сменены, условная маршрутизация "
            "выключена.")
...
def routing_provisioned(tail: str) -> str:
    return ("✅ Обвязка развёрнута, линк до шлюза поднят.\n\nПерезапускаю бота — "
            "интерфейс линка читается при старте. После этого назначь шлюз: "
            "раздел «Условная маршрутизация» → «🛰 Назначить шлюз»."
