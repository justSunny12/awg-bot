...
def settings_routing_text(enabled: bool, status: tuple, mode: str = "home") -> str:
    ...
    head = ("<b>🇷🇺 Условная маршрутизация</b>\n\n"
            "Российские сервисы открываются с российского адреса, остальное — "
            "как обычно. Ссылки и QR-коды у пользователей не меняются.\n")
    ...
    if mode == "abroad":
        mode_block = (
            "\n<b>Режим: по умолчанию за границу.</b>\n"
            "Через дом идут только перечисленные российские сервисы, всё "
            "остальное выходит зарубежным адресом. Домашний канал почти не "
            "нагружен, а отказ шлюза ломает лишь российские сайты.\n"
            "Промах списка заметен сразу: сервис пожалуется на адрес.\n")
    else:
        mode_block = (
            "\n<b>Режим: по умолчанию домой.</b>\n"
            "Через дом идёт всё, кроме перечисленной заграницы. Российские "
            "сервисы работают без списка вовсе, но домашним каналом идёт "
            "основной объём трафика, и отказ шлюза оставляет людей без связи.\n"
            "Промах списка тихий: ломается заграничный сервис, и связать это с "
            "маршрутизацией трудно.\n")
    return (head + mode_block +
            "\n<i>Списки исключений у режимов раздельные — переключение не "
            "перемешивает их и ничего не теряет.</i>\n"
...
def routing_access_line(on: bool) -> str:
    """Строка инфобокса «Доступ к РФ-сервисам». Появляется, как только админ
    разрешил функцию, и показывает состояние переключателя самого клиента —
    чтобы он видел, включено у него сейчас или нет, не заходя в раздел."""
    return f"Доступ к РФ-сервисам: {'🟢 включен' if on else '🔴 выключен'}"


def routing_status_client(ok: bool) -> str:
    """Строка о недоступности шлюза. Показывается ТОЛЬКО когда режим где-то
    включён и при этом сломан: в исправном состоянии о механизме сообщает
    routing_access_line, а дублировать его незачем."""
    return ("🟢 РФ-доступ работает" if ok else
            "🔴 РФ-доступ временно недоступен — сайты открываются с зарубежного адреса")
...
ROUTING_ADD_PROMPT = (
    "Пришли адреса сайтов, которым нужен <b>зарубежный</b> адрес.\n\n"
    "Обычно этого не требуется: заблокированные сайты уже в общем списке, а всё "
    "остальное и так открывается с российского. Добавляй сюда то, что "
    "перестало работать после включения режима.\n\n"
...
def routing_panel_text(*, master_on: bool, domains: list, devices: int,
    ...
    if domains:
        ...
        lines.append(f"\nТвои исключения — всегда через VPN ({len(domains)}):\n{shown}")
    else:
        lines.append("\nИсключений нет. Добавь сюда сайты, которые перестали "
                     "работать после включения режима.")
    return "\n".join(lines)
...
ROUTING_CLEAR_CONFIRM = (
    "Удалить <b>все</b> твои исключения?\n\n"
    "Общий список заблокированных сайтов останется — они продолжат идти через "
    "VPN. Исчезнут только те адреса, которые ты добавил сам."
)
...
def greeting_client(client, server_ok: bool, slots: tuple[int, int] = None,
                    routing_ok: bool = None, routing_access: bool = None) -> str:
    ...
    routing_access=None — строки о РФ-доступе нет вовсе: админ функцию не
    разрешил, и рассказывать о ней тому, кому она недоступна, незачем.
    routing_ok=None — нет отдельной строки о неисправности шлюза; она нужна лишь
    когда режим включён и при этом сломан."""
    ...
    if routing_access is not None:
        text += f"\n{routing_access_line(routing_access)}"
...
# ── Броадкаст ────────────────────────────────────────────────────────────────
BROADCAST_PROMPT = ("📢 <b>Объявление всем</b>\n\nПришли текст — он уйдёт всем "
                    "клиентам и друзьям (кроме тебя). Можно с разметкой "
                    "(<b>жирный</b>, ссылки). Следующим сообщением покажу превью.")
BROADCAST_EMPTY = "Пустой текст. Пришли сообщение для рассылки или нажми «Отмена»."


def broadcast_preview(text: str, n: int) -> str:
    who = "получателю" if n == 1 else "получателям"
    return (f"📢 <b>Превью объявления</b> (так его увидят):\n\n{text}\n\n"
            f"— — —\nБудет отправлено <b>{n}</b> {who}. Отправляем?")
