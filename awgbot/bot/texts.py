...
def broadcast_prompt(names: list, with_friends: bool = False) -> str:
    ...
    return (f"📢 <b>Объявление</b>\n\n"
            ...
            f"Можно с картинками — до {config.TG_ALBUM_MAX} штук, пришли их "
            "перед текстом. Тогда объявление уйдёт одним сообщением: картинки, "
            "текст под ними.\n\n"
...
def broadcast_preview_photos(n: int, names: list = (),
    ...
    who = "человеку" if n == 1 else "людям"
    scope = f"\n{_bc_audience(list(names), with_friends)}." if names else ""
    return (f"👆 Так объявление увидят получатели.\n\n"
            f"Будет отправлено <b>{n}</b> {who}.{scope}\nОтправляем?")


def broadcast_photo_added(total: int) -> str:
    """Отбивка на принятую картинку. Число называем: альбом набирается по одной,
    и без счётчика легко потерять, сколько уже прислано."""
    left = config.TG_ALBUM_MAX - total
    tail = (f"Можно ещё {left}." if left else "Больше не влезет — это максимум.")
    return (f"🖼 Картинка принята: <b>{total}</b> из {config.TG_ALBUM_MAX}. {tail}\n"
            f"Пришли текст объявления — не длиннее {config.TG_CAPTION_MAX} "
            f"символов, потому что он станет подписью к альбому.")


def broadcast_too_many_photos() -> str:
    return (f"🖼 Больше {config.TG_ALBUM_MAX} картинок в одно сообщение Telegram "
            "не берёт — эта не принята. Пришли текст, и отправлю с теми, "
            "что уже есть.")
...
def broadcast_report(names: list, with_friends: bool, delivered: int,
                    failed: int, text: str, photos: int = 0) -> str:
    """Отчёт об отправленном объявлении. Остаётся в чате следом.

    Здесь же ТЕКСТ объявления целиком: сообщение переживает уборку диалога и
    становится единственным следом того, что именно и кому было разослано.
    Отправитель не хранит копию — Telegram показывает ему только свои реплики
    ботам, а не то, что бот разослал другим.
    ...
    if with_friends:
        ...
        line = f"✅ Объявление доставлено {head} {tail}: всего {delivered} {word}."
    else:
        line = f"✅ Объявление доставлено {head}"
    ...
    if failed:
        ...
        line += (f"\n⚠️ Не доставлено {failed} {w} — заблокировали бота "
                 "или удалили аккаунт.")
    if photos:
        # Картинки в отчёт не вложить, поэтому называем их числом и отсылаем к
        # превью: оно осталось в чате и является точной копией разосланного.
        w = plural_ru(photos, "картинка", "картинки", "картинок")
        line += f"\n🖼 Вложено {photos} {w} — объявление целиком видно выше."
