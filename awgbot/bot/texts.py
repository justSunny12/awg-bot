...
def gateway_claim_marked(dev) -> str:
    return (f"🛰 Устройство «{_e(dev.name)}» ({plain_ip(dev.address)}) помечено как шлюз "
            "условной маршрутизации. Конфигурация ниже — перешли её боту шлюза и примени: "
            "без этого он себя шлюзом не считает.")
...
def gateway_replaced(new_dev, old_dev) -> str:
    return (f"🛰 Шлюз теперь «{_e(new_dev.name)}» ({plain_ip(new_dev.address)}); "
            ...
            "2. Конфигурацию ниже перешли боту НОВОГО и примени — до этого он линк "
            "не поднимет.")
...
def gateway_release_forward_text(token: str) -> str:
    return ("🛰 Перешли это сообщение боту шлюза как есть — он опустит линк и выключит "
            f"автозапуск обвязки.\n\n<code>{_e(token)}</code>")
...
def settings_routing_gateway_line(dev) -> str:
    if dev is None:
        return ("\n⚠️ Шлюз не назначен. Назначь устройство админа, стоящее на шлюзе, — "
                "конфигурация для бота шлюза выпустится сразу.")
    return f"\n🛰 Шлюз: «{_e(dev.name)}» ({plain_ip(dev.address)})."


GATEWAY_PICK_INTRO = ("🛰 <b>Назначить шлюз</b>\n\nКакое из твоих устройств стоит на машине-шлюзе? "
                      "Это её клиентский туннель к серверу, выпущенный ботом.")
GATEWAY_PICK_EMPTY = ("🛰 У профиля админа нет устройств, выпущенных ботом. Сначала добавь "
                      "устройство для машины-шлюза и подними на ней его конфиг.")
...
def gateway_mark_ask(dev, prev) -> str:
    ...
    if prev is not None:
        head += (f"\n\nСейчас шлюз — «{_e(prev.name)}». Он станет обычным устройством, а "
                 "боту на нём нужно будет переслать сообщение, которое я дам после смены.")
    return head + "\n\nПосле подтверждения сразу выпущу конфигурацию для бота шлюза."


def gateway_marked(dev) -> str:
...
def gateway_claim_forward_text(token: str, status: str) -> str:
    ...
    return (head + "\n\nПерешли это сообщение основному боту как есть — он найдёт "
            "это устройство по ключу и пометит его шлюзом, а конфигурацию выпустит сразу; "
