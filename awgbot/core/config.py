...
# Клиентский туннель шлюза к ВПС (аплинк) — по нему шлюз является устройством
...
#
# Дефолт — "host": докерная схема снята (docs/ROADMAP.md, п.4), и свежая
# установка без докера с дефолтом "docker" получала бы юнит с
# Requires=docker.service, который systemd не запустит вовсе. Прежний довод
# «отсутствие ключа = сервер, который обновился с докерных времён» исчерпан:
# та установка давно несёт явный host. Докерный режим остаётся только явным.
AWG_RUNTIME = str(_docker.get("runtime") or "host").strip().lower()
...
# Разница между 4096 и 1024 не наша прихоть: 4096-символьная подпись существует,
# но это привилегия Premium-АККАУНТА, а у бота премиума не бывает. Поэтому
# объявление с картинками короче объявления без них — и сказать об этом надо
# ДО того, как админ напишет текст, а не после.
TG_TEXT_MAX = 4096
TG_CAPTION_MAX = 1024
TG_ALBUM_MAX = 10
...
# Единственная модель: НА ШЛЮЗ ИДЁТ ТОЛЬКО ТО, ЧТО В НАБОРЕ. Набор перечисляет
# российские сервисы, которым нужен российский адрес; всё прочее выходит
# зарубежным адресом обычным путём.
#
# Обратная модель («на шлюз всё, чего нет в наборе») существовала и упразднена:
# через домашний канал шёл почти весь трафик по объёму, и его полоса становилась
# общим потолком для всех пользователей. Плюс промах списка там ломал
# ЗАГРАНИЧНЫЙ сервис — молча, потому что связать «сайт не открывается» с
# маршрутизацией почти невозможно. Здесь промах ломает российский сервис, и он
# жалуется вслух: «этот адрес не из России».
#
# Источники дополняют друг друга: пересечение всего 9 доменов из 39, вместе
# покрывают 638.
#   • itdoginfo/allow-domains Russia/outside-* — формат dnsmasq-ipset наш парсер
#     читает как есть. Короткий, но точный: госуслуги, налоговая, РЖД, почта.
