...
class Client:
    ...
    created_at: str
    # Тип профиля (концепт «гость»): owner — обычный, со своей подпиской;
...
@dataclass
class Gateway:
    """Слот шлюза условной маршрутизации (концепт «резервный шлюз»): устройство
    ...
    created_at: str = ""
    lan_mode: int = 0               # 0/1: «за шлюзом — без VPN» (концепт «локальная сеть»)
...
class Device:
    ...
    # Устройство стоит в слоте шлюза условной маршрутизации (концепт «резервный шлюз»):
    ...
    # (концепт «гость»): гость или обычный клиент. Слот, квота, подписка и
