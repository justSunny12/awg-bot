...
def check_fatal() -> None:
    ...
    # host-режим объявлен, но не доделан: перенесён только слой запуска команд,
    # а статус сервиса, SSH-фильтр и вотчдог всё ещё смотрят в docker. Запустись
    # бот в нём — часть функций отвалилась бы поодиночке, и разбирать пришлось бы
    # каждую. Дешевле не стартовать вовсе и сказать почему.
    if config.AWG_RUNTIME == "host":
        problems.append(
            "docker.runtime=host в app.yaml, но перенос с контейнера ещё не "
            "завершён (docs/ROADMAP.md, шаг 2). Верните runtime: docker.")
