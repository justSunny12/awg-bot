"""
watcher.py — вотчдог за конфигом сервера (awg0.conf): чужая правка файла →
реконсиляция состояния бота с ним.

Что наблюдать, решает awg.watch_root(): на хосте это каталог конфигов
интерфейса, в docker-режиме — тот же каталог через /proc/<PID>/root (PID
меняется при рестарте контейнера, ensure_watching() перепривязывается — её
зовёт тик монитора). Два механизма разом: inotify для скорости и mtime-сетка
раз в POLL_NET_SECONDS как страховка на случай, если события не доходят.

Тонкости:
  • дебаунс (правка идёт не одним write) — ждём тишины;
  • игнор самозаписи — awg.is_writing() True, пока бот сам правит файлы, и
    ещё немного после (last_self_write), чтобы сетка не приняла её за чужую.
"""
...
class AwgWatcher:
    ...
    inotify может не пробить namespace/overlayfs на конкретном ядре (в docker-
    режиме путь идёт через /proc/<PID>/root) — поэтому параллельно раз в
    POLL_NET_SECONDS сверяем mtime файлов через os.stat по тому же пути. Это
    системный вызов (микросекунды, ноль сабпроцессов). Худшая задержка при
