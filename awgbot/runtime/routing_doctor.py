...
# Протухший хендшейк. WireGuard пересогласуется через REKEY_AFTER_TIME (120 с) и
# считает сессию негодной после REJECT_AFTER_TIME (180 с). Берём с запасом:
# короткий провал связи не должен краснить исправный линк, а вот минуты молчания
# при работающем keepalive — это уже не «может быть», а «та сторона не отвечает».
_STALE_HANDSHAKE = 300
...
def _probe_layers() -> list[tuple[str, str, str]]:
    ...
    elif age > _STALE_HANDSHAKE:
        # Раньше здесь было безусловное «Линк поднят» — зелёным, лишь бы хендшейк
        # вообще существовал. Трёхчасовой хендшейк при REJECT_AFTER_TIME в 180 с
        # означает мёртвый линк, а слой докладывал, что всё хорошо, и внимание
        # уезжало на шлюз. Возраст сам по себе плохая мера живости (потому рядом
        # и стоит зонд), но ПРОТУХШИЙ хендшейк — уже не «может быть», а факт:
        # туннель не пересогласуется, значит с той стороны никто не отвечает.
        out.append((_BAD, f"Линк молчит: хендшейку {age} с",
                    f"Пересогласование идёт каждые ~2 минуты, значит сторона "
                    f"шлюза не отвечает. Чинить НА ШЛЮЗЕ: поднят ли интерфейс "
                    f"(awg-quick up), доходит ли он до нас. Адрес шлюза в линке: "
                    f"{peer or 'не определён'}"))
    else:
    ...
    # ── 7. учёт РФ-трафика — только чтение ──
    out += _probe_rf_acct()
    return out
...
def _mb(n: int) -> str:
    ...
    Целочисленно: плавающая точка на границе давала лишнюю
    сотую."""
