...
async def test_mode_switch_is_exclusive_and_applies_at_once(services, fake_bot, monkeypatch):
    """Режимы взаимоисключающие, и переключение применяется сразу.

    Правила описывают режим; отложи применение до тика — и экран будет говорить
    одно, а маркировка делать другое. Между ними человек успеет решить, что фича
    сломана.
    """
    ...
    written: dict = {}
    monkeypatch.setattr(st, "set_value", lambda k, v: written.__setitem__(k, v))
    monkeypatch.setattr(config, "routing_default_route",
                        lambda: written.get("app.routing.default_route", "home"))
    applied: list = []
    monkeypatch.setattr(services, "reconcile_routing", lambda: applied.append(1))

    cb, nav = _acb(fake_bot)
    await sh.routing_action(cb, SetCB(sec="rt", act="do", key="mode", val="abroad"), services)
    assert written["app.routing.default_route"] == "abroad"
    assert applied, "переключение не применилось немедленно"

    # повтор того же режима ничего не переписывает и не пересобирает
    applied.clear()
    cb2, _ = _acb(fake_bot)
    await sh.routing_action(cb2, SetCB(sec="rt", act="do", key="mode", val="abroad"), services)
    assert applied == []


async def test_unknown_mode_is_refused(services, fake_bot, monkeypatch):
    """Значение приходит из callback_data — принимать что попало нельзя.

    routing_default_route() падает обратно на home при мусоре, но записать мусор
    в yaml значит оставить админу конфиг, который не делает того, что в нём
    написано."""
    from awgbot.bot.handlers import settings as sh
    from awgbot.bot.callbacks import SetCB
    ...
    monkeypatch.setattr(st, "set_value", lambda k, v: written.__setitem__(k, v))
    cb, _ = _acb(fake_bot)
    await sh.routing_action(cb, SetCB(sec="rt", act="do", key="mode", val="куда-нибудь"), services)
    assert written == {}
