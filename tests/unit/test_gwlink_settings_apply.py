"""
Настройки с сервера по каналу линка — сторона шлюза (концепт «канал линка»,
этап 2, §3.1, §4.7, §10.1): закрытый список ключей, строгая проверка значений,
...
def test_the_list_of_keys_is_closed_and_changes_only_on_purpose():
    ...
    assert gwlink.SETTINGS_KEYS == ("ADMIN_IPS", "HOME_SUBNETS", "LAN_MODE", "RESOLVER"), (
        "список ключей канала изменился без правки теста — см. концепт §3.1, §10.1")
...
def test_settings_matching_the_unit_touch_nothing(svc, unit):
    """Миграция (§9.2): первое сообщение после включения канала несёт те же
