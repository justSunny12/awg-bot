...
FULL = "непредвиденная ошибка, файл своих списков не записан: нет места на диске"
...
def test_a_different_refusal_restarts_the_wait(agent, clock):
    ...
    assert _due(agent, OWN) is True
    clock.at(AFTER + 100)
    agent._nudge_note(OWN, "непредвиденная ошибка, файл своих списков не записан: диск только для чтения")
    clock.at(2 * AFTER + 99)
...
@pytest.mark.parametrize("err", [
    "свои списки — обвязка перевыставляется",
    "свои списки — обвязка перевыставится в ближайшие минуты",
])
...
@pytest.mark.parametrize("err", [
    "файлов своих списков нет — раздел не применился",
    "скрипта своих списков нет — перевыпусти конфигурацию шлюза",
    "скрипта записей SMB нет — перевыпусти конфигурацию шлюза",
    "не пройдена проверка строк",
    "dnsmasq --test отверг списки — откатываю",
    "скрипт своих списков не ответил за 150 с",
    # «непредвиденная ошибка» не в начале — чужой хвост, не поломка агента
    "rc=1: непредвиденная ошибка",
])
def test_a_refusal_that_is_not_a_breakage_is_never_nudged(agent, clock, err):
    """Просьба — только при поломке на шлюзе («непредвиденная ошибка…»):
