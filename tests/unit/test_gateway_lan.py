"""
Агент шлюза: локальная сеть без VPN (docs/gateway-lan.md §3.5) — блок панели,
...
def _lan_on(monkeypatch, *, home=None, active=True, up=True, status=None, own=(2, 1),
    ...
    monkeypatch.setattr(gwguard, "lan_status", lambda: lists or {"domains": "1180", "nets": "412",
                                                                  "updated_at": "2026-09-20T10:00:00+05:00", "rc": "0"})
    monkeypatch.setattr(gwguard, "lan_own_lists", lambda: own)
    monkeypatch.setattr(gwguard, "ipv6_disabled", lambda iface: v6)
...
def test_lan_status_block_and_checks_when_healthy(svc, monkeypatch):
    ...
    assert info["domains"] == 1180 and info["nets"] == 412 and info["resolved"] == 50
    ...
    assert by["IPv6 на LAN"].ok is True
...
def test_router_redirect_is_judged_by_counter_growth(svc, monkeypatch):
    ...
    шести часов — нет, с временем последнего пакета и отсылкой к рецепту."""
    ...
    last["dns_at"] = timeutil.to_iso(timeutil.now() - datetime.timedelta(hours=7))
...
def test_missing_table_and_dead_resolver_are_named(svc, monkeypatch):
    _lan_on(monkeypatch, home=None, active=False, up=None)
    _, checks = svc.lan_status()
    by = {c.name: c for c in checks}
    assert by["таблица локальной сети"].ok is False and "перевыпусти конфигурацию" in by["таблица локальной сети"].detail
    assert by["резолвер"].ok is False and "journalctl -u dnsmasq" in by["резолвер"].detail
    assert by["апстрим через аплинк"].ok is None, "резолвер лежит — апстрим не проверяли"
...
def test_status_carries_lan_block_and_panel_shows_it(svc, monkeypatch):
    ...
    st = GwStatus(link_up=True, handshake_age=5.0, checks=[GwCheck("резолвер", True)],
                  lan={"iface": "end0", "addr": "192.168.68.222", "resolver": "10.9.1.1", "domains": 1180,
                       "nets": 412, "resolved": 50, "updated_at": "2026-09-20T10:00:00+05:00",
    ...
    assert "1180 доменов, 412 подсетей, 50 адресов по резолву" in out and "свои: 2 в туннель, 1 напрямую" in out
    st.checks.append(GwCheck("трафик с роутера", False, "пакетов нет"))
...
def test_lists_job_warns_after_two_failures_only(svc, monkeypatch):
    ...
    assert svc.lan_lists_update() == [] and svc.db.get_state(svc._LAN_FAILS_KEY) == "0"
    monkeypatch.setattr(gwguard, "lan_mode", lambda: False)
...
def test_home_table_info_parses_counters_and_sets(monkeypatch):
    ...
    assert info["sets"] == {"lan_vpn4": 2, "lan_vpn_nets4": 0}
