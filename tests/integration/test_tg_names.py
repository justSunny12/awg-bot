...
async def test_refresh_job_fills_names_and_survives_api_refusals(services, make_active_client):
    ...
    assert n == 1
    assert services.db.get_client(a.id).tg_name == "Артём П." and services.db.get_client(a.id).tg_name_at
...
async def test_middleware_stamps_account_name_on_every_role(services, make_active_client):
    ...
    assert fresh.tg_name == "U" and fresh.tg_name_at
...
def test_links_prefer_account_name_and_fall_back_to_profile(services, make_active_client):
    ...
    services.db.update_client_fields(owner.id, tg_name="Вася Пупкин")
    services.db.update_client_fields(res.holder.id, tg_name="Артём")
    dev = services.db.get_device(dc.device_id)
    assert 'Получено от <a href="tg://user?id=3130">Вася Пупкин</a>' in texts.held_device_card(dev, 0)
    ...
    assert 'владелец: <a href="tg://user?id=3130">Вася Пупкин</a>' in texts.greeting_guest("Артём", True, donor, 1)
