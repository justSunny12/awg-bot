...
#                          Шлюз по файлу первого применения — ещё метка
#                          $DATA_DIR/.fresh-install: первый запуск агента скажет
#                          серверу «установлен», даже если база уцелела.
...
#                          На шлюзе — ещё firewall.env и свои списки локальной
...
apply_gw_bundle() {  # применить бандл: аплинк, линк, обвязка, файервол шлюза
    local f="$1"
    echo; log "─── Конфигурация с сервера AWG: $f ───"
    sh "$f" || die "файл первого применения не применился — смотри вывод выше; исправь причину и повтори установку"
    # бандл удаляет себя сам после успеха; на случай старого бандла — добиваем
    [[ -f "$f" ]] && rm -f -- "$f" && log "файл конфигурации удалён: внутри были ключи"
    ...
    if [[ "$role" == "gateway" ]]; then
        ...
        if [[ -n "$bundle" ]]; then
            ...
            if gw_bundle_secrets "$bundle"; then
                from_bundle=1
                # первый запуск агента должен сказать серверу «установлен», даже
                # если база от прежней установки уцелела: метка, снимает агент
                mkdir -p "$DATA_DIR" && : > "$DATA_DIR/.fresh-install"
            else
...
ensure_awg_generation() {
    ...
    if [[ -n "$mig_if" ]]; then
        # Переезд УЖЕ идёт — и он не наш: двойники на нём рождены под прежнее
        # поколение. Объявить его целью нового значило бы на финале записать
        # хост перешедшим на ядро, которого он не видел. Цель не трогаем;
        # смена поколения ждёт финала, о ней напомнит сам бот.
        warn "поколение $want ждёт: идёт переезд на $mig_if, он обязан завершиться первым"
        log "после финала бот предложит подготовить переезд на новое поколение"
        return 0
    fi
...
cmd_lan() {
    # Локальная сеть без VPN на шлюзе (концепт «локальная сеть», функция A): личные
...
cmd_gw_bundle() {
    ...
    # --link <if>: бандл другого слота (резервный шлюз, концепт «резервный шлюз»);
