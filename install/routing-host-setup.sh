...
# Разовая подготовка хоста под условную маршрутизацию.
#
# СТАВИТСЯ ОДИН РАЗ — и на горячую не меняется: править базовый NAT боевого
# сервера ради фичи неоправданно. Бот разворачивает обвяз одной кнопкой, а
...
#
# КТО ЗАПУСКАЕТ. С v2.14.0 — сам бот, кнопкой «Развернуть» в разделе условной
# маршрутизации (services.routing_provision). Руками — для разбора и отката.
#
# ЗАПУСК:
...
# Дефолт — host, как у бота (config.AWG_RUNTIME). Разойдись они — скрипт при
# нечитаемом app.yaml уходил бы в контейнерную ветку, ждал контейнер и падал
# на «не удалось узнать адрес контейнера» там, где контейнера нет и не должно.
AWG_RUNTIME="${AWG_RUNTIME:-host}"
...
if [ -n "$RESOLVER_ADDR" ]; then
    # Апстримы, защиты и однократные ключи (bind-interfaces, cache-size) ведёт
    # резолвер бота. Наше — только адрес перехвата, и то лишь если он не
    # совпадает с адресом резолвера.
...
#
# Без этого перехвата фича просто не работает у ушедшего в DoH: набор наполняет
# ТОЛЬКО наш резолвер, и клиент, спрашивающий мимо нас, не добавит в него ничего.
# Отказ при этом заметен (российские сервисы ругаются на адрес), но объясним он
# лишь отсюда — со стороны это «у всех работает, а у меня нет».
...
if ! systemctl list-unit-files 2>/dev/null | grep -q "^${DNSMASQ_SERVICE}\.service"; then
    ...
    say ""
    # Код 1, а не 0: дальше идут DNAT, FORWARD, DoT и sysctl, без которых
    # обвязка нерабочая. Нулём вызывающий (бот, юнит) читал это как успех и
    # рапортовал «✅ обвязка развёрнута» при неразвёрнутой обвязке.
    say "Выхожу с ошибкой: без юнита dnsmasq обвязка неполная."
    exit 1
fi
...
say "    dig +short @$DNS_ADDR gosuslugi.ru >/dev/null"
say "    ipset list vpn_u<N> | grep -c '^[0-9]'"
say "  Домен обязан быть ИЗ РОССИЙСКОГО списка: набор перечисляет то, чему нужен"
say "  российский адрес, и gosuslugi.ru после резолва там БУДЕТ. Проверять по"
say "  заграничному домену бессмысленно — его в наборе нет и быть не должно."
...
say "здесь лежит не только маршрутизация, но и MASQUERADE клиентской подсети,"
say "то есть после ребута интернет пропадёт у ВСЕХ клиентов, а не у включённых."
say ""
