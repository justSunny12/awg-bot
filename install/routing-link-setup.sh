...
HDREOF
        ...
        # Кому открыт SSH на шлюз через туннель — устройства админа. Список
        # знает только бот ВПС: он передаёт его окружением при сборке, бандл
        # вшивает, юнит на шлюзе закрепляет. Сменился состав — новый бандл
        # (бот сам напомнит).
        printf 'SSH_ALLOW="${SSH_ALLOW:-%s}"\nexport SSH_ALLOW\n' "$(printf '%s' "${SSH_ALLOW:-}" | tr -cd '0-9./ ')"
