...
#      обычная строка лога и код возврата 0.
...
if [[ -x "$INSTALL_DIR/venv/bin/python" ]]; then
    ...
    printf '  2) Удалить прошлую установку вместе с данными и настройками, затем чистая установка\n' >&2
    printf '  3) Отмена\n' >&2
    ...
    case "${__ch:-3}" in
        ...
        2)
            printf '%s[install:!]%s Будут удалены: код, /etc/awg-bot (секреты и конфиг), /var/lib/awg-bot (БД, копии)' "$c_err" "$c_off" >&2
            if [[ "$inst_role" == "gateway" ]]; then
                printf ', линк до сервера AWG, юнит и таблицы обвязки, /etc/awg-gw, /var/lib/awg-gw, /opt/awg-gw. Аплинк остаётся.\n' >&2
            else
            ...
            read -r -p "Точно удалить и поставить заново? [y/N]: " __a < /dev/tty
            [[ "${__a,,}" == "y" ]] || { log "отмена — ничего не изменено"; exit 0; }
            if [[ "$inst_role" != "gateway" ]]; then
                read -r -p "Напиши слово УДАЛИТЬ, чтобы подтвердить, что клиенты останутся без доступа: " __a < /dev/tty
                [[ "$__a" == "УДАЛИТЬ" ]] || { log "отмена — ничего не изменено"; exit 0; }
            fi
            wipe_previous
            log "прошлая установка снята — ставлю заново" ;;
        *)
            # отмена — не ошибка: обычная строка установщика и код 0
            if [[ "$inst_role" == "gateway" ]]; then
                log "отмена — ничего не изменено. Новую конфигурацию на установленном агенте применяют из чата бота шлюза (переслать файл) или так: sudo sh <файл> (без --install)"
            else
                log "отмена — ничего не изменено (для действий: sudo awg-bot update|restore|uninstall)"
            fi
            exit 0 ;;
    esac
fi
...
if [[ -e "$INSTALL_DIR" ]]; then
    printf '%s[install:!]%s найден остаток прошлой установки в %s (без рабочего venv).\n' "$c_err" "$c_off" "$INSTALL_DIR" >&2
