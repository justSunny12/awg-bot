...
if [ -n "$OURS" ]; then
    say "  Старый страж AWGBOT_SSH — НЕ трогаем: его снимет сам бот после"
    say "  включения файервола (awg-bot firewall setup), когда SSH уже держит"
    say "  таблица awg_bot_guard. До этого строка безвредна: на хосте её цель"
    say "  (дефолтный шлюз) — роутер провайдера, правило ничего не закрывает."
fi
if [ -n "$FOREIGN" ]; then
    say ""
